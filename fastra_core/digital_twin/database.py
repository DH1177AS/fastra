    ...
    def save_progress_entry(self, entry: ProgressEntry) -> None:
        ...
        try:
            orm = ProgressEntryORM(
                ...
                report_date=entry.report_date.isoformat() if isinstance(entry.report_date, datetime) else str(entry.report_date),
                report_type=entry.report_type,
                period_start=entry.period_start.isoformat() if isinstance(entry.period_start, datetime) else str(entry.period_start),
                period_end=entry.period_end.isoformat() if isinstance(entry.period_end, datetime) else str(entry.period_end),
    ...
    def save_change_order(self, vo: ChangeOrder) -> None:
        ...
        try:
            orm = ChangeOrderORM(
                ...
                request_date=vo.request_date.isoformat() if isinstance(vo.request_date, datetime) else str(vo.request_date),
    ...
    def save_archive_record(self, archive: ArchiveRecord) -> None:
        ...
        try:
            orm = ArchiveRecordORM(
                ...
                archived_at=archive.archived_at.isoformat() if isinstance(archive.archived_at, datetime) else str(archive.archived_at),
